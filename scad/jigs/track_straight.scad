		...
		difference() {
			...
			if (straight != "none") {
				...
				if (double_sided_rails) {
					translate([ radius, 0, wood_height() ])
					rotate([ 180, 0, 90 ])
					wood_rails(straight_length);
				}
			}
			...
			if (right != "none") {
				...
				wood_rails_arc(radius, angle);
				if (double_sided_rails) {
					translate([ radius * 2 + wood_width(), 0, wood_height() ])
					rotate([ 180, 0, 180 ])
					wood_rails_arc(radius, angle);
				}
			}
...
module straight_with_drill_holes() {
	color(SPRUCE_WOOD) difference() {
		...
		rotate([ 0, 90, 0 ])
		magnet_hole();
		translate([ s_mh_xpos, s_mh_ypos1, s_mh_zpos ])
		rotate([ 0, -90, 0 ])
		magnet_hole();
		translate([ 0, s_mh_ypos2, s_mh_zpos ])
		rotate([ 0, 90, 0 ])
		magnet_hole();
		translate([ s_mh_xpos, s_mh_ypos2, s_mh_zpos ])
		rotate([ 0, -90, 0 ])
		magnet_hole();
	}
}
